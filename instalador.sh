...
URL_SQUASHFS="${1:-https://github.com/StickArcade/mymusic/releases/latest/download/jukebox.squashfs}"
...
export BIOS_ARQUIVOS_DIR="$BIOS_ARQUIVOS_DIR"
alias jk='cd "\$BIOS_ARQUIVOS_DIR"'
alias juckebox='cd "\$BIOS_ARQUIVOS_DIR" && { batocera-es-swissknife --emukill; sleep 2; python3 g900bios.rom; }'
alias jklog='tail -f /userdata/system/.dev/jukebox.log'
alias jkcred='python3 "\$BIOS_ARQUIVOS_DIR/g900util.rom" --config \$JUKEBOX_DIR/config.json'
